...
# It looks like it works until the radio button is switched from ggplot to baseR without 
# changing any of the other options.
...
ui <- fluidPage(
  tags$style("#plot_type_panel { display:none; }"),
  ...
  tabsetPanel(
    id = "plot_type_panel",
    tabPanel("baseR", 
             plotOutput(outputId = "iris_baseR_plot", width = "400px")),
    tabPanel("ggplot", 
             plotOutput(outputId = "iris_ggplot", width = "400px"))
  ),
...
server <- function(input, output, session) {
  ...
  output$iris_ggplot <- renderPlot(ggplot_object())
  
  output$iris_baseR_plot <- renderPlot({
  ...
  observeEvent(input$plot_type, {
    updateTabsetPanel(session, "plot_type_panel", selected = input$plot_type)
  })
