...
???
We can start the UI code by calling the function page_fillable(). This is actually from the bslib package but has taken over as standard way of creating layouts. There are other options that we'll come on to look at, but this essentially means the app will fill the available space.
So page_fillable is a function, and to that we pass the elements that we want to be displayed in the app. We’ll go through the components in more detail later, but let’s have a quick look now.  
